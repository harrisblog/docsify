...
!> 注意：此题我认为表达模棱两可，题库正确答案表示此处的对角线是正对角线，而我把正副对角线都考虑进去了，考试时就按照正对角线。另外注意还有对角线以上的元素。
...
!> 注意：变量名一定要用题目给的，否则关键词匹配度不高，从而扣分

---

题目：请编一个函数void fun(int tt\[M][N], int pp[N]), tt指向一个M行N列的二维数组，求出二维数组每列中最大元素，并依次放入pp所指的一维数组中。
	...
	for (i = 0; i < *n;) {
		if (bb[i] == y) {
			...
			*n = *n - 1; //不能*n--
			...
			i++; // 不能随意i++，万一移过来的数还是要删除的呢？所以要再判断一下，不等于则再来i++
		}
	}
  /**********  End  **********/
}
